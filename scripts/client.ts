...
import {
  ErrorOverlay,
  overlayId,
} from '../node_modules/vite/src/client/overlay'
...
function createErrorOverlay(err: ErrorPayload['err']) {
  ...
  clearErrorOverlay()
  document.body.appendChild(new ErrorOverlay(err) as any)
}

function clearErrorOverlay() {
  document.querySelectorAll(overlayId).forEach((n) => (n as any).close())
}
    ...
    Array.from(modulesToUpdate).map(async (dep) => {
      ...
      try {
        const newMod = await import(
          /* @vite-ignore */
          `${base + path.slice(1)}.js?import&t=${timestamp}${
            query ? `&${query}` : ''
          }`
        )
